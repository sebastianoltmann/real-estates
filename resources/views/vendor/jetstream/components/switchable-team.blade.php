...
<x-dynamic-component :component="$component" href="#" onclick="event.preventDefault();
    ...
    <div class="d-flex align-content-center overflow-hidden">
        @if (Auth::user()->isCurrentProject($team))
    ...
    <form method="POST" action="{{ route( (isAdminRequest() ? 'admin.' : ''). 'current-project.update') }}" id="switch-team-form-{{ $team->id }}">
        @method('PUT')
        @csrf
